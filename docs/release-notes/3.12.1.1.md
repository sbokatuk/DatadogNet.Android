...
The MAUI sample in this repository does not currently build. Supporting `net8.0-android34.0` forces
every AndroidX binding to the newest version that still ships a net8 asset, and that older
Navigation line still depends on the standalone `SavedState.Ktx` artifact that AndroidX has since
merged — which collides with the newer `savedstate` MAUI 9 resolves. The bindings themselves are
unaffected and are proven end to end on an emulator by the device tests. Dropping net8 resolves it.
