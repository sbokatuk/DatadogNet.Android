...
public class PackageLayoutTests
{
    ...
    [Theory]
    [MemberData(nameof(Packages.Names), MemberType = typeof(Packages))]
    public void Every_package_ships_consumer_keep_rules(string name)
    {
        using var package = Packages.OpenPackage(name);

        // Every Datadog entry point is reached from .NET through JNI alone - no Java code
        // references it - so a consumer's R8 shrink removes it and the binding throws
        ...
        // in every package, not just the two that shipped them first. The .pro files are
        // generated by build/generate-r8-rules.sh (upstream's consumer rules, which .NET for
        // Android never reads out of the .aar, plus curated keeps for the JNI-only entry
        // surface), and the r8 leg of the emulator matrix exercises them functionally.
        Assert.NotNull(package.GetEntry($"buildTransitive/{Packages.PackageId(name)}.targets"));
        Assert.NotNull(package.GetEntry($"buildTransitive/{Packages.PackageId(name)}.pro"));
    }
